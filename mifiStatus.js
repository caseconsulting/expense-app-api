    ...
    _.forEach(records, record => {
        if (record.eventName === 'INSERT') {
            ...
            if (record.dynamodb.NewImage.mifiStatus === 'false') {
                eventText = record.dynamodb.NewImage.firstName + record.dynamodb.NewImage.lastName + ' (employee Id: ' + record.dynamodb.NewImage.id + ') has been created without a mifi benefit.';
                //TODO: create an expense for the person
                params = {
                    Message: eventText, 
                    Subject: "Mifi status change",
                    TopicArn: "arn:aws:sns:us-east-1:453274522834:MifiStatus" //TODO: figure out how to reference ARN of cloudformation resource
                };
            ...
            if (record.dynamodb.OldImage.mifiStatus === null || record.dynamodb.OldImage.mifiStatus === 'true') {
                if (record.dynamodb.NewImage.mifiStatus === 'false') { //TODO: will it just come back as a boolean?
                    ...
                    eventText = record.dynamodb.NewImage.firstName + record.dynamodb.NewImage.lastName + ' (employee Id: ' + record.dynamodb.NewImage.id + ') has opted out of the mifi benefit.';
                    //TODO: create an expense for the person
                    params = {
                        Message: eventText, 
                        Subject: "Mifi status change",
