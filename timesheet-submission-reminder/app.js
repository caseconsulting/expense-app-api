...
const {
  isCaseReminderDay: _isCaseReminderDay,
  shouldSendCaseEmployeeReminder: _shouldSendCaseEmployeeReminder
} = require('./case-helpers.js');
...
const TEST_EMPLOYEE_NUMBERS = ['20017', '20016'];
 ...
 * @returns {Promise<string[]>} The array from employee numbers to send reminders to
 */
 ...
 *
 * @returns {Promise<any[]>} The array of portal employees
 */
 ...
 *
 * @returns {Promise<any[]>} The array of portal contracts
 */
...
async function _logMessageReminder(employee) {
  ...
  // TODO: create audit in aurora database
 ...
 * @returns {string} The SMS formatted phone number
 */
 ...
 *
 * @param {any[]} portalEmployees - The array of portal employee objects
 */
 ...
 *
 * @param {*} employee The employee to send a reminder to
 * @param {*} isCaseReminderDay Result of _isCaseReminderDay() to decide which message to send
 * @returns {*} The SNS client response
 */
async function _sendReminder(employee, isCaseReminderDay) {
  // decide on reminder text based on type of reminder being sent
  let reminders = {
    week: `CASE Alerts: Your contract requires time entries to be submitted each week. Please log into QuickBooks and
    complete your time entries for this week as soon as possible.`,
    month: `This is a reminder that you have not met the minimum hour requirement for this month.  Please be sure to
    enter and submit your hours as soon as possible to keep payroll running smoothly.`
  };
 ...
 * @param {string} attribute Attribute that will be updated
 * @param {string} tableName Name of table in which the dynamoObj is in
